      ...
      <div className="flex items-center gap-2 max-w-[500px] mx-auto">
        ...
        >
          {mainItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon size={16} strokeWidth={2} className={active ? "text-white" : "text-[#888]"} aria-hidden="true" />
                {active && (
        ...
        <div className="flex gap-1.5 shrink-0">
          <button
            ...
            aria-label="Notificações e Histórico"
            aria-current={isNotification ? "page" : undefined}
          ...
          >
            ...
            >
              <Bell size={16} strokeWidth={2} className={isNotification ? "text-white" : "text-[#888]"} aria-hidden="true" />
            </div>
            {!isNotification && (
              <div className="absolute top-2.5 right-2.5 w-[14px] h-[14px] rounded-full bg-[#E85D5D] flex items-center justify-center">
                <span className="text-[8px] text-white" aria-hidden="true">1</span>
              </div>
            )}
          </button>
          ...
          >
            <div className="w-[42px] h-[44px] rounded-[18px] flex items-center justify-center bg-[#222]">
              <LogOut size={16} strokeWidth={2} className="text-[#E85D5D]" aria-hidden="true" />
            </div>
          </button>
        </div>
      </div>
